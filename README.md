...
| **昆明水务** | 昆明市自来水用水数据查询，支持阶梯水价 | 13个 | 配置时获取* |
...
- 查看水表基本信息（口径、抄表周期）
- 显示最新账单用水量和费用明细
- 自动计算水费构成（自来水费、污水费、垃圾费）
- 支持两月用水量和月平均用水量统计

> **注意**：本集成在配置时获取一次数据，不会自动更新。如需更新数据，请重新配置集成。
...
| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `user_code` | 账户编号 | - | 水费账户户号 |
| `user_name` | 用户名 | - | 账户注册姓名 |
| `address` | 用水地址 | - | 自来水使用地址 |
| `caliber` | 水表口径 | - | 水表规格口径 |
| `cycle` | 抄表周期 | - | 抄表周期说明 |
...
| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `latest_usage` | 最新账单用水量 | m³ | 最新一期账单用水量 |
| `two_month_usage` | 两月总用水量 | m³ | 两个月累计用水量 |
| `monthly_avg_usage` | 月平均用水量 | m³ | 月平均用水量（两月÷2） |

#### 💰 费用类

| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `latest_bill_amount` | 最新账单总金额 | CNY | 最新账单总费用（估算） |
| `bill_water_fee` | 自来水费 | CNY | 自来水费用（3.20元/m³） |
| `bill_sewage_fee` | 污水费 | CNY | 污水处理费用（1.00元/m³） |
| `bill_garbage_fee` | 垃圾费 | CNY | 垃圾处理费用（固定20.00元） |

#### 📅 日期类

| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `bill_date` | 最新账单日期 | - | 最新账单日期（每月10日） |
...
> **注意**：当前版本费用计算采用固定单价（自来水3.20元/m³ + 污水1.00元/m³），未使用阶梯价格。
...
| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `balance` | 表端余额 | CNY | 燃气表可用余额 |
| `gas_company` | 所属燃气公司 | - | 云南中石油昆仑燃气有限公司昆明分公司 |
| `user_code` | 户号 | - | 燃气账户户号 |
| `customer_name` | 用户名 | - | 账户注册姓名 |
| `address` | 用气地址 | - | 燃气使用地址 |
...
| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `owe_amount` | 待上表金额 | CNY | 待上表金额 |
| `last_payment` | 最近缴费金额 | CNY | 最近一次缴费金额 |
| `last_payment_date` | 最近缴费时间 | - | 最近一次缴费时间 |

#### 📅 月度数据类

| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `monthly_volume` | 上月用气量 | m³ | 上月度用气量 |
| `monthly_cost` | 上月用气费用 | CNY | 上月度燃气费 |
| `current_month_volume` | 本月用气量 | m³ | 当月累计用气量 |
| `current_month_cost` | 本月用气费用 | CNY | 当月累计燃气费用 |
...
| `yearly_cost` | 年度用气费用 | CNY | 本年累计燃气费用 |
| `recent_monthly_usage` | 近31天累计用量 | m³ | 最近31天累计用气 |
| `recent_monthly_cost` | 近31天用气费用 | CNY | 最近31天累计费用 |

#### 📆 日数据类

| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `last_day_usage` | 最近一日用气量 | m³ | 最新日期用气量 |
| `last_day_usage_time` | 最近用气日期 | - | 最新用气数据日期 |
| `last_day_usage_cost` | 最近一日用气费用 | CNY | 最新日期燃气费用 |

#### 🪜 阶梯价格类

| 传感器ID | 中文名称 | 单位 | 说明 |
|---------|---------|------|------|
| `ladder_stage` | 当前阶梯档位 | - | 当前阶梯档位(1-3) |
| `ladder_unit_price` | 当前阶梯单价 | CNY/m³ | 当前阶梯单价 |

#### 💡 阶梯气价说明（昆明民用）

| 阶梯 | 年用气范围 | 单价 |
|-----|-----------|------|
| 1 | 0-360 m³/年 | 2.97 元/m³ |
| 2 | 360-540 m³/年 | 3.56 元/m³ |
| 3 | 540 m³以上/年 | 4.46 元/m³ |

**费用计算采用分段累加**：
```
例如：400 m³ 费用 = 360×2.97 + (400-360)×3.56
```
...
- **昆明水务**：配置时获取一次，不会自动更新。如需更新，请重新配置集成
