#!/usr/bin/env python3
"""
HA-Grid-South 传感器数据测试脚本
用于测试南网电网集成，获取并展示所有传感器数据
"""
...
def test_login_with_sms() -> CSGClient:
    """使用短信验证码登录"""
    ...
    try:
        client.api_send_login_sms(PHONE_NUMBER)
        print("✅ 验证码已发送到您的手机！")
        ...
        print("  1. 手机号格式错误")
        print("  2. 网络连接问题")
        print("  3. 南网 API 服务暂时不可用")
        sys.exit(1)

    # 获取验证码输入
    print("\n" + "-" * 40)
    sms_code = input("请输入收到的 6 位验证码: ").strip()
...
def main():
    """主函数"""
    print("\n" + "█" * 60)
    print("█" + " " * 20 + "HA-Grid-South 传感器测试" + " " * 18 + "█")
    print("█" * 60)

    # 尝试加载已保存的 session
    if os.path.isfile("session.json"):
        print("\n⚠️  检测到已保存的 session.json")
        use_saved = input("是否使用已保存的登录态？(y/N): ").strip().lower()
        if use_saved == 'y':
            print("\n正在加载已保存的登录态...")
            try:
                ...
                if client.verify_login():
                    ...
                    print("❌ 已保存的登录态已失效，需要重新登录")
                    client = test_login_with_sms()
            except Exception as e:
